...
async function printOrder(order) {
  ...
  const response = await fetch('https://api.printnode.com/printjobs', {
    ...
    body: JSON.stringify({
      ...
      title: `Pizzahaus Pinocchio Bestellung #${order.orderNum}`,
      contentType: 'raw_base64',
      content: base64Content,
      source: 'Pizzahaus Pinocchio Backend'
    })
  });
...
function buildReceipt(order) {
  ...
  // Header
  receipt += CENTER;
  receipt += DOUBLE_ON + BOLD_ON + 'PINOCCHIO' + LF + DOUBLE_OFF + BOLD_OFF;
  receipt += 'Bahnhofstr. 6 · 59320 Ennigerloh' + LF;
  receipt += 'Tel: 0 25 24 / 95 17 49' + LF;
  receipt += SEPARATOR;
